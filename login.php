...
if (isset($_POST['btn_dangnhap'])) {
  ...
  $stmt_kh = $conn->prepare(
    "SELECT * FROM khachhang WHERE tendangnhap = ? AND matkhau = ?"
  );

  if ($stmt_kh) {

    $stmt_kh->bind_param("ss", $tendangnhap, $matkhau);
    $stmt_kh->execute();
  ...
  // Nếu không phải khách hàng thì kiểm tra quản trị viên
  $stmt_ad = $conn->prepare(
    "SELECT * FROM quantrivien WHERE tendangnhap = ? AND matkhau = ?"
  );

  if ($stmt_ad) {

    $stmt_ad->bind_param("ss", $tendangnhap, $matkhau);
    $stmt_ad->execute();
...
if (isset($_POST['btn_dangky'])) {
  ...
  $stmt_check = $conn->prepare(
    "SELECT * FROM khachhang WHERE tendangnhap = ?"
  );
  ...
  if ($stmt_check) {
    ...
    if ($check_res->num_rows > 0) {
      ...
      // Thêm tài khoản bằng prepared statement
      $stmt_insert = $conn->prepare(
        "INSERT INTO khachhang 
                (tenkh, tendangnhap, matkhau, sdt) 
                VALUES (?, ?, ?, ?)"
      );
